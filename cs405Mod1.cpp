...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    {
        if (result > (std::numeric_limits<T>::max() - increment)) { //overflow check, sees if result is greater than the difference of max() and increment, which would overflow
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
	    ...
	    if (result < (std::numeric_limits<T>::lowest() + decrement)) { // underflow check, checked the C++ documentation and lowest() works as min() for integrals and as -max() for floating points, so it makes it work for both use cases
...
void test_overflow()
{
	...
	// try/catch for overflow_error then outputs accordingly a message of its result if successful or overflow occured
    ...
    try {
        auto result = add_numbers<T>(start, increment, steps + 1);
            std::cout << +result << std::endl;
    }
    catch (const std::overflow_error) {
        std::cout << "OVERFLOW OCCURED!!" << std::endl;
...
void test_underflow()
{
    ...
    const T decrement = std::numeric_limits<T>::max() / steps; // decrement doesnt apply to all types since it would ahve to be double this, since some types are negative and some are non-negative
    // whats our starting point
    ...
    // try/catch for underflow_error then outputs accordingly a message of its result if successful or underflow occured
    std::cout << "\tSubtracting Numbers Without Overflow (" << +start << ", " << +decrement << ", " << steps << ") = ";
    try { // try/catch for underflow
    ...
    std::cout << "\tSubtracting Numbers With Overflow (" << +start << ", " << +decrement << ", " << (steps + 6) << ") = "; // added more steps for the decrement since we start at the max and need to get to the min
    try { //try/catch for underflow
        auto result = subtract_numbers<T>(start, decrement, steps + 6); // since 5 steps to reach midway it would double that plus 1 for underflow, so 11 in total
            std::cout << + result << std::endl;
    }
    catch (const std::underflow_error) {
        std::cout << "UNDERFLOW OCCURED!!" << std::endl;
